...
class UnikeyTests(unittest.TestCase):
    ...
    # Re-opening a temporary file is tricky,
    # so the next two tests are ignored at the moment.
    # This works on Ubuntu with CPython, but not PyPy.

    def ignore_file_standard(self):
        unikey.create(self.output_filename, ['standard.htxt'])
        with open(self.output_filename, 'r', encoding='utf-8') as output_file:
            text = output_file.read()
            assert text == self.result + '\n'

    def ignore_file_extension(self):
